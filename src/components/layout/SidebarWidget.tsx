'use client'

import React from 'react'

export default function SidebarWidget() {
  return (
    <div className="mx-auto mb-10 w-full max-w-60 rounded-2xl bg-gray-50 px-4 py-5 text-center dark:bg-white/[0.03]">
      <h3 className="mb-2 font-semibold text-gray-800 dark:text-white/90">
        Atnaujink i Pro
      </h3>
      <p className="mb-4 text-gray-500 text-theme-sm dark:text-gray-400">
        Gauk daugiau funkciju ir galimybiu
      </p>
      <a
        href="#"
        className="flex items-center justify-center p-2 font-medium text-white rounded-lg bg-brand-500 text-theme-sm hover:bg-brand-600"
      >
        Atnaujinti dabar
      </a>
    </div>
  )
}
